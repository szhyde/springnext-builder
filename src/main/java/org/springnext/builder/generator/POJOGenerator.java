...
public class POJOGenerator extends Generator {
     ...
     * @param table 表格对象
     */
    public POJOGenerator(Table table,ProjectInfo projectInfo) {
        // 调用父类构造函数
        super(table,projectInfo);
        // 初始化类的类型(类名).
        this.classType = new Type(projectInfo.getPackageName()+"."+projectInfo.getModelName()+".entity." + getTableName());
    }
